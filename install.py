...
def get_venv_path():
    ...
    return os.path.join(venv_dir, "dubbed_venv")
...
def install_app(venv_path):
    print("Installing Dubbed into the virtual environment...", end="")
    sys.stdout.flush()
    pip_path = os.path.join(venv_path, "bin", "pip")
    subprocess.run(
        [pip_path, "install", "dubbed"],
        check=True,
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
    )
...
def get_icon(venv_path):
    ...
    icon_relative_path = "dubbed/resources/images/dubbed-128.png"
...
def get_app_path(venv_path):
    return os.path.join(venv_path, "bin", "dubbed")
...
def create_desktop_file(icon, version, python, app):
    ...
    Type=Application
    Name=Dubbed
    Comment=Username generator that never leaves your machine.
    Exec={python} {app}
    Icon={icon}
    Terminal=false
    Categories=Utility;
    ...
    with open(
        os.path.expanduser("~/.local/share/applications/dubbed.desktop"), "w"
    ) as f:
